...
The package `bertopicr` is based on the Python package `BERTopic` by Maarten Grootendorst (https://github.com/MaartenGr/BERTopic). The package `bertopicr` introduces functions to display topic model results of `BERTopic` models in `R`. The R package and the tutorial were created by using the programming advice of `OpenAI`'s language model `GPT4o`. 
