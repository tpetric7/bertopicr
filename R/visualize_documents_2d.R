...
#' @param reduced_embeddings A matrix or data frame of reduced-dimensionality embeddings (2D). Typically generated using UMAP.
...
#' @param filename A character string specifying the name of the HTML file to save the visualization. Default is "visualize_documents_2d".
...
#'   auto_open = TRUE)
#' }
visualize_documents_2d <- function(
    model,
    texts,
    reduced_embeddings,
    custom_labels = FALSE,
  ...
  # Ensure reduced_embeddings has 2 columns for 2D plotting
  if (ncol(reduced_embeddings) != 2) {
    stop(
      "The 'reduced_embeddings' argument must have 2 dimensions (2 columns) for 2D plotting."
    )
  }
